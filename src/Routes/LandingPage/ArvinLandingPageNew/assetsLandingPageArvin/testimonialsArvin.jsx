export const testimonialsArvin = [
  {
    title: "Organização e foco no ensino",
    description:
      "Consigo organizar minhas turmas e acompanhar o progresso dos alunos de forma prática e eficiente, focando no que realmente importa no ensino.",
    name: "Yara Calligari",
    role: "Professora particular",
    img: "https://ik.imagekit.io/vjz75qw96/assets/icons/Testemunhos/e8f66c2c1b9b5782ff3477a9d8a5f46a13642620.png",
  },
  {
    title: "Ensino personalizado e ágil",
    description:
      "A plataforma centraliza as aulas e relatórios me dando flexibilidade para adaptar o conteúdo conforme a necessidade de cada aluno.",
    name: "Arthur Vincent",
    role: "Professor particular",
    img: "https://ik.imagekit.io/vjz75qw96/assets/icons/Testemunhos/73755670b8f877b2c6038843022986e2071c7228.png",
  },
  {
    title: "Os alunos amam!",
    description:
      "A plataforma está sendo um recurso muito bom e tem facilitado tanto as minhas aulas quanto no estudo de casa dos alunos. Eles estão gostando bastante!",
    name: "Victor Leonardo",
    role: "Professor particular",
    img: "https://ik.imagekit.io/vjz75qw96/assets/icons/Testemunhos/vic.png",
  },
  {
    title: "Meu trabalho melhorou muito",
    description:
      "A plataforma Arvin é dinâmica e excelente para ensinar; meus alunos amaram aprender de forma interativa!",
    name: "Gabriel Isaías",
    role: "Professor particular",
    img: "https://ik.imagekit.io/vjz75qw96/assets/icons/Testemunhos/gab.png",
  },
];
